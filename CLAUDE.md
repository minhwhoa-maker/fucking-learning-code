...
- `driver.html` — owner quản lý tài xế, tính lương theo tháng, export Excel/PDF. Bảng có 9 cột: Tên | Email | SĐT | Số chuyến | Tổng lương | Tạm ứng | Thực nhận | **Đang giữ** | Xóa. Click tên tài xế → `openTripsModal()` (filter theo tháng đang chọn). Click cột "Đang giữ" → `openBalanceModal(driverId, driverName)` hiện breakdown công nợ (2 section: "Tạm ứng chuyến" từ `trips` và "Tạm ứng ngoài chuyến" từ `tam_ung_thang`, kèm dòng tổng). Nút `+ Tạm ứng` → `openAdvanceModal()` (modal form, dot-format input với `numberToVietnamese` hint — pattern tương tự `driver-page.html`) INSERT vào `tam_ung_thang` với `trang_thai: 'confirmed'`, luôn dùng tháng hiện tại (`new Date().toISOString().slice(0,7)`). `loadDrivers()` chạy 4 query song song: trips filter theo tháng (lương/số chuyến), advances filter theo tháng (cột Tạm ứng), allTrips all-time `trang_thai='hoan_thanh'` (balance), allAdvances all-time (balance). **Balance** = Σ`tam_ung`(trips hoàn thành) + Σ`tam_ung_thang` − Σ`hoan_ung`(trips hoàn thành); màu đỏ nếu > 0, xanh nếu ≤ 0. `addDriver()` check trùng email + SĐT qua `maybeSingle()` trước INSERT, include `owner_id: ownerProfileId`.
